...
var qClassCacheFlush = 1 << 15
var ClassINET = 1

// Because the mdns pkt is intercepted by the lower level
// So open this function to let core pass mdns query to this package
exports._handleITxPTQuery = function _handleITxPTQuery(server, query, opts) {
  if (typeof query !== 'object') return
  server._handleITxPTQuery(query, opts)
}

// setText is used to response the mdns aggressively
exports.setText = function setText (server, cb) {
  ...
  var records = flatten(services
    ...
    .map(function (service) {
      ...
      records.forEach(function (record) {
        // Set cache flush
        if (record.type !== 'PTR') record.class = qClassCacheFlush | ClassINET
      })
      return records
    }))
  if (records.length === 0) return
  // send the records aggressively
  server.mdns.respond(records, cb)
}
